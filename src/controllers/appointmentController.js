const Appointment = require("../models/AppointmentsSchema.js");
...
// 📌 Book Appointment (no auth dependency for now)
exports.bookAppointment = async (req, res) => {
  try {
    ...
    // ✅ Require patientId in body
    if (!patientId) {
      return res.status(400).json({ message: "Please provide patientId in request body." });
    }

    // Validate patient exists
    const patientExists = await Patient.findOne({ patient_id: patientId });
    if (!patientExists) {
